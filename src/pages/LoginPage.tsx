...
import { LogIn, UserPlus, Phone, Mail, Lock, User, Scissors, Check, Sparkles, KeyRound } from 'lucide-react';
import { useApp } from '../context/AppContext';

export const LoginPage: React.FC = () => {
  const { login, registerUser, setActivePage, addToast } = useApp();
  ...
  const handleQuickDemoLogin = async () => {
    setIsLoading(true);
    try {
      await login('cliente.vip@barba10.com.br', '123456');
      setActivePage('agenda');
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <div className="pb-24 pt-4 px-4 max-w-md mx-auto space-y-6 animate-fadeIn">
      ...
      <div className="text-center space-y-2 pt-2">
        <div className="w-14 h-14 rounded-2xl bg-[#DAA520] text-black font-extrabold flex items-center justify-center mx-auto shadow-xl shadow-[#DAA520]/20 border border-[#DAA520]/40">
          <Scissors className="w-7 h-7 stroke-[2.2]" />
        </div>
        <h1 className="text-2xl font-black text-white font-mono tracking-tight">
          BARBA10 <span className="text-[#DAA520]">CLIENTES</span>
        </h1>
      ...
      <div className="text-center text-[10px] text-gray-500 font-mono uppercase tracking-wider">
        <p>Barbearia BARBA10 • Seus dados estão protegidos</p>
      </div>
